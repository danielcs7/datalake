...
def download_parquet_from_minio(parquet_filename):
    ...
    try:
        response = minio_client.get_object(bucket_name, f"landzone/landzone/{parquet_filename}")
        df_existing = pd.read_parquet(BytesIO(response.data))
        response.close()
        response.release_conn()
        logging.info(f"Download do arquivo {parquet_filename} do MinIO concluído.")
        return df_existing
    except S3Error:
...
def upsert_and_upload_parquet(df_new, table_name, db_name):
    ...
    logging.info(f"Arquivo {parquet_filename} atualizado e enviado para MinIO.")
